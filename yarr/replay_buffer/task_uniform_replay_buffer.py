...
class TaskUniformReplayBuffer(UniformReplayBuffer):
    ...
    def _add(self, kwargs: dict):
        ...
        with self._lock:
            ...
            with self._add_count.get_lock():
                # add_final() stores a padding row with terminal=-1; it must not be
                # task-indexed or is_valid_transition() will always reject it
                # (see UniformReplayBuffer.is_valid_transition).
                term = kwargs[TERMINAL]
                if term != -1:
                    task = kwargs[TASK]
                    if task not in self._task_idxs:
                        self._task_idxs[task] = [cursor]
                    else:
                        self._task_idxs[task].append(cursor)
    ...
    def sample_index_batch(self,
        ...
        if not tasks:
            raise RuntimeError(
                "TaskUniformReplayBuffer has no task indices yet (_task_idxs is empty). "
                "Ensure demos were loaded into the replay buffer before training."
            )
        attempt_count = 0
        ...
        while not found_indicies and attempt_count < self._max_sample_attempts:
            ...
            for task in sampled_tasks:
                ...
                raw = list(self._task_idxs[task])
                task_data_size = len(raw)
                if task_data_size == 0:
                    attempt_count += 1
                    continue
                num_samples = math.ceil(task_data_size / self._num_replicas)
                total_size = num_samples * self._num_replicas
                task_indices = raw[self._rank:total_size:self._num_replicas]
                if len(task_indices) == 0:
                    # Fewer transitions than replicas: strided shard is empty; fall back to all indices.
                    task_indices = raw

                sampled_task_idx = int(np.random.choice(task_indices, 1)[0])
        ...
        if len(indices) != batch_size:
            raise RuntimeError(
                ...
                ' valid indices. Batch size is {}'.
                    format(attempt_count, len(indices), batch_size))
